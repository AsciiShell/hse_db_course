...
Для реализации проверки можно использовать триггеры BEFORE INSERT и BEFORE UPDATE (\imref{img:insertScheduler}).
Данные выводятся с помощью команды SELECT.

\begin{figure}[H]
	\centering		
	\includegraphics[width=\linewidth]{image/triggerScheduler}
	\caption{Пример триггера для проверки расписания}\label{img:triggerScheduler}
\end{figure}
